...
const images = [
  "/images/commercial/commercial-warehouse.jpg",
]
    ...
    <main>
      ...
      <section className="pt-32 pb-12" style={{backgroundColor:"#1B2B8A"}}>
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <h1 className="mt-4 text-3xl font-black text-white sm:text-4xl">Warehouse Painting Gallery</h1>
          <p className="mt-2 text-blue-200 text-sm">Photos from our warehouse painting projects across Mumbai, Thane & Bhiwandi.</p>
        </div>
      </section>
            ...
            {images.map((img, i) => (
              <div key={i} className="overflow-hidden rounded-xl shadow-sm">
                <div className="relative h-64">
                  <Image src={img} alt={`Warehouse Painting project ${i + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-300" sizes="(max-width: 639px) 100vw, (max-width: 1023px) 50vw, 33vw" />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-10 rounded-xl bg-white border border-gray-200 p-6 text-center">
            <p className="text-sm" style={{color:"#6b7280"}}>More photos coming soon. Want to see warehouse painting examples in person?</p>
            <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/services/commercial" className="text-sm font-semibold" style={{color:"#1B2B8A"}}>View Warehouse Painting Service Details →</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16" style={{backgroundColor:"#1B2B8A"}}>
        <div className="mx-auto max-w-4xl px-6 lg:px-8 text-center">
          <h2 className="text-2xl font-black text-white sm:text-3xl">Want Warehouse Painting for Your Property?</h2>
          <p className="mt-3 text-blue-200 text-sm">Get a free site visit and written quotation today.</p>
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            <a href="https://wa.me/919158800517?text=Hi%2C%20I%20need%20a%20quote%20for%20Warehouse%20Painting." target="_blank" rel="noopener noreferrer"
